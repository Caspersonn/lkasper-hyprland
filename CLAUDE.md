...
*Blog gate (auto-create).* Before anything else, check openspec status --change "<name>" --json. If the blog artifact is not done:
...
jj describe
the changelog should also be updated. Each entry headline (the - **name** — summary bullet) MUST be <= 80 characters; put detail and See [proposal]/[ADR] links on indented sub-bullets below it, not in the headline.
