...
### Requirement: Urgency-aware auto-dismiss

A toast SHALL auto-dismiss after a timeout for `LOW` and `NORMAL` urgency
(honoring the notification's expire timeout when provided, otherwise a default
of about 5 seconds), and a `CRITICAL` toast SHALL remain until dismissed by the
user or closed by the sender.

#### Scenario: Normal notification auto-dismisses
- **GIVEN** a `NORMAL` urgency toast is shown
- **WHEN** the timeout elapses with no interaction
- **THEN** the toast is dismissed

#### Scenario: Critical notification stays
